...
Point::Point(float x, float y) { pos = prevPos = initPos = Vec2(x, y); }
...
void Point::Update(float deltaTime, float drag, const Vec2 &acceleration,
                   float elasticity, Mouse *mouse, int windowWidth,
                   int windowHeight) {
  Vec2 cursorToPosDir = pos - mouse->GetPosition();
  float cursorToPosDist =
      cursorToPosDir.x * cursorToPosDir.x + cursorToPosDir.y * cursorToPosDir.y;
  ...
  Vec2 newPos = pos + (pos - prevPos) * (1.0f - drag) +
                acceleration * (1.0f - drag) * deltaTime * deltaTime;
  prevPos = pos;
  pos = newPos;
